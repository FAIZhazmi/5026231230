        ...
        <tbody>
            @foreach($keranjang as $item)
                <tr>
                    ...
                    <td class="text-center">{{ number_format($item->Harga_per_item, 0, ',', '.') }}</td>
                    <td class="text-center">{{ number_format($item->Total_Jumlah_x_Harga * $item->Harga_per_item, 0, ',', '.') }}</td>
                    <td class="text-center">
                        <!-- Tombol Hapus (Batal) -->
                        <form action="{{ route('keranjang.destroy', $item->Kode_Pembelian) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
